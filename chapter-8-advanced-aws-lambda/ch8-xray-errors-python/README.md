...
```bash
ch8-xray-errors-python$ sam build --use-container
```
...
```bash
ch8-xray-errors-python$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
ch8-xray-errors-python$ sam local start-api
ch8-xray-errors-python$ curl http://localhost:3000/
```
...
```bash
ch8-xray-errors-python$ sam logs -n HelloWorldFunction --stack-name ch8-xray-errors-python --tail
```
...
```bash
ch8-xray-errors-python$ pip install -r tests/requirements.txt --user
# unit test
ch8-xray-errors-python$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
ch8-xray-errors-python$ AWS_SAM_STACK_NAME="ch8-xray-errors-python" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "ch8-xray-errors-python"
```
